...
DumpGridGZ::DumpGridGZ(LAMMPS *lmp, int narg, char **arg) : DumpGrid(lmp, narg, arg), writer(lmp)
{
  if (!compressed) error->all(FLERR,"Dump grid/gz only writes compressed files");
}

/* ---------------------------------------------------------------------- */

void DumpGridGZ::openfile()
{
  ...
  if (append_flag && access(filecurrent, F_OK) == 0) {
    error->all(FLERR, "Dump grid/gz file {} already exists and cannot be appended to", filecurrent);
  }
...
void DumpGridGZ::write_header(bigint ndump)
{
...
void DumpGridGZ::write_data(int n, double *mybuf)
{
  if (buffer_flag) {
...
int DumpGridGZ::modify_param(int narg, char **arg)
{
